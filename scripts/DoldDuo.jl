...
for i ∈ 1:length(x), j ∈ 1:length(t)
    ...
    u[j,i] = ForwardDiff.derivative(xint,t[j]);
    yint = interpolate((t,),y_o[:,i],Gridded(Linear()));
    v[j,i] = ForwardDiff.derivative(yint,t[j]);
end
=#
for i=1:length(x)
    u[:,i] = grad(x_o[:,i],t);
...
gif(anim, "Plots/velocity.gif", fps = 6)
end
...
function grad(vector,points)
    dvdp = zeros(length(vector));
    for i = 2:length(vector)-1
        dvdp[i] = 0.5*(vector[i+1]-vector[i-1])/(points[i+1]-points[i-1]);
    end
